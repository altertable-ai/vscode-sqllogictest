...
export function activate(context: vscode.ExtensionContext) {
  const maybeSetSqllogictest = (doc: vscode.TextDocument) => {
    ...
    const closedAt = context.workspaceState.get(doc.fileName) as
      | number
      | undefined;
    const closedRecently = closedAt && closedAt + 50 >= Date.now();
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(maybeSetSqllogictest)
  );
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(maybeSetSqllogictest)
  );
  context.subscriptions.push(
